...
namespace Pr21_II_15
{
    public class BinaryTree
    {
        ...
        private class Node
        {
            ...
            public static int SumUpToLevel(Node root, int k, int currentLevel = 1)
            {
                if (root == null || currentLevel > k)
                    return 0;

                int currentValue = Convert.ToInt32(root.inf);
                return currentValue +
                       SumUpToLevel(root.left, k, currentLevel + 1) +
                       SumUpToLevel(root.right, k, currentLevel + 1);
            }
        ...
        // метод для подсчета количества узлов со степенью 1
        public int SumUpToLevel(int k)
        {
            return Node.SumUpToLevel(tree, k);
        }
